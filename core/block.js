...
/**
 * Return all classes referenced by this block.
 * @return {!Array.<string>} List of object names.
 */
/*Blockly.Block.prototype.getObjs = function() {
  var objects = [];
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldObject) {
        objects.push(field.getValue());
      }
    }
  }
  return objects;
 ...
 * @param {string} newName Renamed variable.
 */
/*Blockly.Block.prototype.renameObj = function(oldName, newName) {
  for (var i = 0, input; input = this.inputList[i]; i++) {
    for (var j = 0, field; field = input.fieldRow[j]; j++) {
      if (field instanceof Blockly.FieldObject &&
          Blockly.Names.equals(oldName, field.getValue())) {
